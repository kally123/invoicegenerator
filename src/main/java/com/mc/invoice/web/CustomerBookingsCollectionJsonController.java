...
@RooJSON
@RestController
@RequestMapping(value = "/api/customerbookings", name = "CustomerBookingsCollectionJsonController", produces = MediaType.APPLICATION_JSON_VALUE)
public class CustomerBookingsCollectionJsonController {
    ...
    public static UriComponents listURI() {
        return MvcUriComponentsBuilder.fromMethodCall(MvcUriComponentsBuilder.on(CustomerBookingsCollectionJsonController.class).list(null, null)).build().encode();
    }
    ...
    public ResponseEntity<?> createBatch(@Valid @RequestBody Collection<CustomerBooking> customerBookings, BindingResult result) {
        if (result.hasErrors()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(result);
        }
    ...
    public ResponseEntity<?> updateBatch(@Valid @RequestBody Collection<CustomerBooking> customerBookings, BindingResult result) {
        if (result.hasErrors()) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(result);
        }
        getCustomerBookingService().save(customerBookings);
