...
public class AccomidationRepositoryImpl extends QueryDslRepositorySupportExt<Accomidation>
		implements AccomidationRepositoryCustom {
	...
	public Page<Accomidation> findAll(GlobalSearch globalSearch, Pageable pageable) {
		...
		Path<?>[] paths = new Path<?>[] { accomidation.accomidationType, accomidation.price,
				accomidation.customerBooking };
		applyGlobalSearch(globalSearch, query, paths);
		AttributeMappingBuilder mapping = buildMapper().map(ACCOMIDATION_TYPE, accomidation.accomidationType)
				.map(PRICE, accomidation.price).map(CUSTOMER_BOOKING, accomidation.customerBooking);
