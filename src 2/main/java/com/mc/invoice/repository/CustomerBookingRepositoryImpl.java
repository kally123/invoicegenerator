...
public class CustomerBookingRepositoryImpl extends QueryDslRepositorySupportExt<CustomerBooking> implements CustomerBookingRepositoryCustom {
    ...
    public Page<CustomerBooking> findAll(GlobalSearch globalSearch, Pageable pageable) {
        ...
        AttributeMappingBuilder mapping = buildMapper().map(CUSTOMER, customerBooking.customer).map(ACCOMIDATION, customerBooking.accomidation).map(NO_OF_DAYS, customerBooking.noOfDays).map(TOTAL_PRICE, customerBooking.totalPrice).map(CUSTOMER_DISCOUNT, customerBooking.customerDiscount);
